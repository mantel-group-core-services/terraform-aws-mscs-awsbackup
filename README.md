...
|------|-------------|------|---------|:--------:|
| <a name="input_additional_backup_plan_config"></a> [additional\_backup\_plan\_config](#input\_additional\_backup\_plan\_config) | Contains the general configuration of an AWS Backup Plan. This can be used to set any number of arbitrary additional Backup Plans. | <pre>map(object({<br>    cold_storage_after       = optional(number)<br>    completion_window        = number<br>    enable_continuous_backup = bool<br>    enable_vss               = bool<br>    name                     = string<br>    retention_in_days        = number<br>    schedule                 = string<br>    selection_tag_key        = string<br>    selection_tag_value      = string<br>    start_window             = number<br>    additional_selection_tags = optional(list(object({<br>      key   = string<br>      value = string<br>      type  = string<br>    })))<br>  }))</pre> | `{}` | no |
...
| <a name="input_backup_plan_name_prefix"></a> [backup\_plan\_name\_prefix](#input\_backup\_plan\_name\_prefix) | Prefix to prepend to all backup plan names (e.g., 'myapp' results in 'myapp\_backupDaily'). | `string` | `""` | no |
| <a name="input_continuous_backup_plan_config"></a> [continuous\_backup\_plan\_config](#input\_continuous\_backup\_plan\_config) | Contains the general configuration of the Continuous AWS Backup Plan. | <pre>object({<br>    completion_window   = number<br>    enabled             = bool<br>    name                = string<br>    retention_in_days   = number<br>    schedule            = string<br>    selection_tag_key   = string<br>    selection_tag_value = string<br>    start_window        = number<br>  })</pre> | <pre>{<br>  "completion_window": 180,<br>  "enabled": true,<br>  "name": "backupContinuous",<br>  "retention_in_days": 3,<br>  "schedule": "cron(0 * ? * * *)",<br>  "selection_tag_key": "backup",<br>  "selection_tag_value": "Continuous",<br>  "start_window": 60<br>}</pre> | no |
| <a name="input_daily_backup_plan_config"></a> [daily\_backup\_plan\_config](#input\_daily\_backup\_plan\_config) | Contains the general configuration of the Daily AWS Backup Plan. | <pre>object({<br>    cold_storage_after  = optional(number)<br>    completion_window   = number<br>    enabled             = bool<br>    enable_vss          = bool<br>    name                = string<br>    retention_in_days   = number<br>    schedule            = string<br>    selection_tag_key   = string<br>    selection_tag_value = string<br>    start_window        = number<br>    additional_selection_tags = optional(list(object({<br>      key   = string<br>      value = string<br>      type  = string<br>    })))<br>  })</pre> | <pre>{<br>  "completion_window": 180,<br>  "enable_vss": false,<br>  "enabled": true,<br>  "name": "backupDaily",<br>  "retention_in_days": 35,<br>  "schedule": "cron(0 10 ? * * *)",<br>  "selection_tag_key": "backup",<br>  "selection_tag_value": "Daily",<br>  "start_window": 60<br>}</pre> | no |
| <a name="input_hourly_backup_plan_config"></a> [hourly\_backup\_plan\_config](#input\_hourly\_backup\_plan\_config) | Contains the general configuration of the Hourly AWS Backup Plan. | <pre>object({<br>    cold_storage_after  = optional(number)<br>    completion_window   = number<br>    enabled             = bool<br>    enable_vss          = bool<br>    name                = string<br>    retention_in_days   = number<br>    schedule            = string<br>    selection_tag_key   = string<br>    selection_tag_value = string<br>    start_window        = number<br>    additional_selection_tags = optional(list(object({<br>      key   = string<br>      value = string<br>      type  = string<br>    })))<br>  })</pre> | <pre>{<br>  "completion_window": 180,<br>  "enable_vss": false,<br>  "enabled": true,<br>  "name": "backupHourly",<br>  "retention_in_days": 14,<br>  "schedule": "cron(0 * ? * * *)",<br>  "selection_tag_key": "backup",<br>  "selection_tag_value": "Daily",<br>  "start_window": 60<br>}</pre> | no |
| <a name="input_iam_path"></a> [iam\_path](#input\_iam\_path) | The IAM path for the role and policy. | `string` | `"/mgms/"` | no |
| <a name="input_iam_policy_name"></a> [iam\_policy\_name](#input\_iam\_policy\_name) | The name of the IAM policy attached to the AWS Backup service role. | `string` | `"mg_cs_backup_role_policy"` | no |
| <a name="input_iam_role_name"></a> [iam\_role\_name](#input\_iam\_role\_name) | The name of the IAM role used by AWS Backup. | `string` | `"mgms_cs_backup_role"` | no |
| <a name="input_kms_key_alias"></a> [kms\_key\_alias](#input\_kms\_key\_alias) | The alias name for the KMS key used to encrypt the backup vault. | `string` | `"alias/mgms_backup_key"` | no |
| <a name="input_monthly_backup_plan_config"></a> [monthly\_backup\_plan\_config](#input\_monthly\_backup\_plan\_config) | Contains the general configuration of the Monthly AWS Backup Plan. | <pre>object({<br>    cold_storage_after  = optional(number)<br>    completion_window   = number<br>    enabled             = bool<br>    enable_vss          = bool<br>    name                = string<br>    retention_in_days   = number<br>    schedule            = string<br>    selection_tag_key   = string<br>    selection_tag_value = string<br>    start_window        = number<br>    additional_selection_tags = optional(list(object({<br>      key   = string<br>      value = string<br>      type  = string<br>    })))<br>  })</pre> | <pre>{<br>  "completion_window": 480,<br>  "enable_vss": false,<br>  "enabled": true,<br>  "name": "backupMonthly",<br>  "retention_in_days": 455,<br>  "schedule": "cron(0 19 1 * ? *)",<br>  "selection_tag_key": "backup",<br>  "selection_tag_value": "Monthly",<br>  "start_window": 60<br>}</pre> | no |
...
| <a name="input_unscoped_backup_plan_config"></a> [unscoped\_backup\_plan\_config](#input\_unscoped\_backup\_plan\_config) | Contains the configuration of the Unscoped backup plan for untagged resources. | <pre>object({<br>    completion_window        = number<br>    enabled                  = bool<br>    enable_continuous_backup = bool<br>    enable_vss               = bool<br>    name                     = string<br>    retention_in_days        = number<br>    schedule                 = string<br>    selection_tag_key        = string<br>    start_window             = number<br>  })</pre> | <pre>{<br>  "completion_window": 180,<br>  "enable_continuous_backup": false,<br>  "enable_vss": false,<br>  "enabled": true,<br>  "name": "mgms_unscoped_backup_plan",<br>  "retention_in_days": 14,<br>  "schedule": "cron(0 10 ? * * *)",<br>  "selection_tag_key": "backup",<br>  "start_window": 60<br>}</pre> | no |
...
| <a name="input_weekly_backup_plan_config"></a> [weekly\_backup\_plan\_config](#input\_weekly\_backup\_plan\_config) | Contains the general configuration of the Weekly AWS Backup Plan. | <pre>object({<br>    cold_storage_after  = optional(number)<br>    completion_window   = number<br>    enabled             = bool<br>    enable_vss          = bool<br>    name                = string<br>    retention_in_days   = number<br>    schedule            = string<br>    selection_tag_key   = string<br>    selection_tag_value = string<br>    start_window        = number<br>    additional_selection_tags = optional(list(object({<br>      key   = string<br>      value = string<br>      type  = string<br>    })))<br>  })</pre> | <pre>{<br>  "completion_window": 480,<br>  "enable_vss": false,<br>  "enabled": true,<br>  "name": "backupWeekly",<br>  "retention_in_days": 105,<br>  "schedule": "cron(0 10 ? * FRI *)",<br>  "selection_tag_key": "backup",<br>  "selection_tag_value": "Weekly",<br>  "start_window": 60<br>}</pre> | no |
| <a name="input_yearly_backup_plan_config"></a> [yearly\_backup\_plan\_config](#input\_yearly\_backup\_plan\_config) | Contains the general configuration of the Yearly AWS Backup Plan. | <pre>object({<br>    cold_storage_after  = optional(number)<br>    completion_window   = number<br>    enabled             = bool<br>    enable_vss          = bool<br>    name                = string<br>    retention_in_days   = number<br>    schedule            = string<br>    selection_tag_key   = string<br>    selection_tag_value = string<br>    start_window        = number<br>    additional_selection_tags = optional(list(object({<br>      key   = string<br>      value = string<br>      type  = string<br>    })))<br>  })</pre> | <pre>{<br>  "completion_window": 480,<br>  "enable_vss": false,<br>  "enabled": true,<br>  "name": "backupYearly",<br>  "retention_in_days": 765,<br>  "schedule": "cron(0 19 1 1 ? *)",<br>  "selection_tag_key": "backup",<br>  "selection_tag_value": "Yearly",<br>  "start_window": 60<br>}</pre> | no |
